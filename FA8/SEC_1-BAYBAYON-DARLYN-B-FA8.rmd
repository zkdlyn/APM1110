...
```
 ...
 (a\) What is the probability that the signal will exceed 224 microvolts? **P(X>224)**
```{r}
1- pnorm(224, mean, sd)
```

 (b\) What is the probability that the signal will be between 186 and 224 microvolts? **P(186<X<224)**
```{r}
pnorm(224, mean, sd) - pnorm(186, mean, sd)
```
 (c\) What is the micro voltage below which 25% of the signals will be? **P(X<=k)=0.25**
```{r}
qnorm(0.25, mean, sd)
```
 (d\) What is the probability that the signal will be less than 240 microvolts, given that it is larger than 210 microvolts? **P(X<240|X>210)**
```{r}
...
```
 (f\) What is the probability that the signal will be less than 220 microvolts, given that it is larger than 210 microvolts? **P(X<220|X>210)**
```{r}
